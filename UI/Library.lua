local startUpArgs = { "universal", "public" }
-- [[
--obleus remake
...
local services = setmetatable({}, {
	__index = function(self, Name)
		local Success, Result = pcall(game.FindService, game, Name)
...
do
	...
	function utility.findtriggers(text)
		local triggers = {
			...
			["{day}"] = os.date("%d"),
			["{year}"] = os.date("%Y"),
			["{fps}"] = services.Stats.FrameRateManager:FindFirstChild("RenderAverage")
					and string.format("%.1f", 1000 / services.Stats.FrameRateManager.RenderAverage:GetValue()) .. " fps"
				or "0 fps",
			["{ping}"] = services.Stats ~= nil and math.floor(
...
local library = {
	...
	settings = { folder_name = "seere/" .. startUpArgs[1], default_accent = Color3.fromRGB(255, 255, 255) },
	drawings = {},
...
function library.createpicker(cfg)
	...
	local colorpickertypes =
		library.object_colorpicker_inner(default, defaultalpha, holder, colorpickers, flag, callback, -6)
...
function library.object_textbox(box, text, callback, finishedcallback)
	box.MouseButton1Click:Connect(function()
		services.ContextActionService:BindActionAtPriority("disablekeyboard", function()
			return Enum.ContextActionResult.Sink
...
function library:new(cfg)
	...
	function window:page(cfg)
			...
			local function handleoptionclick(option, button)
				button.MouseButton1Click:Connect(function()
					...
					if selected_plr ~= last_plr then
						...
						local data = services.HttpService:JSONDecode(response).data
		...
		function page:server_list(cfg)
			...
			local flag = cfg.flag or cfg.Flag or utility.nextflag()
			local servers = services.HttpService:JSONDecode(request({
				Url = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=100"):format(
					game.PlaceId
				),
				Method = "GET",
			}).Body).data
			...
			join_frame.MouseButton1Click:Connect(function()
				task.spawn(function()
					if clicked_join then
						...
						--
						services.TeleportService:TeleportToPlaceInstance(game.PlaceId, library.flags[flag])
						--
					else
						clicked_join = true
						counting_join = true
		...
		function page:section(cfg)
			...
			function section:toggle(cfg)
				...
				function toggle:colorpicker(cfg)
					...
					toggle.colors += 1
					local cp =
						library.object_colorpicker(default, defaultalpha, holder, toggle.colors - 1, flag, callback, -6)
					function colorpicker_tbl:set(color)
					...
					library:connect(services.InputService.InputBegan, function(inp)
						if (inp.KeyCode == key or inp.UserInputType == key) and not binding then
							if key_mode == "Hold" then
								...
								c = library:connect(services.RunService.RenderStepped, function()
									if callback then
										callback(true)
									end
								end)
			...
			function section:colorpicker(cfg)
				...
				local colorpickertypes =
					library.object_colorpicker(default, defaultalpha, holder, colorpickers, flag, callback, -6)
			...
			function multisection:section(cfg)
				...
				function section:toggle(cfg)
					...
					function toggle:colorpicker(cfg)
						...
						toggle.colors += 1
						local cp = library.object_colorpicker(
							default,
							defaultalpha,
							holder,
							toggle.colors - 1,
